...
function BlockFormatDropDown({
  ...
  return (
    <Dropdown options={options} onOptionChange={(option: Value) => {
      switch (option.value) {
        case "paragraph":
          formatParagraph();
          break;
    ...
    }} initialValue={"normal"} btnLabel={(value) => value.icon} btnClassName="rounded-r-[5px]"/>
  );
}
    ...
    if ($isRangeSelection(selection)) {
      ...
      if (elementDOM != null) {
        if ($isListNode(anchorNode)) {
          ...
          const type = $isHeadingNode(anchorNode)
            ? anchorNode.getTag()
            : anchorNode.getType();
