        ...
        (template) => {
          editor.update(() => {
            ...
            for (let i = 0; i < itemsCount; i++) {
              container.append($createLayoutItemNode(items[i]).append($createParagraphNode()));
            }
      ...
      editor.registerCommand(
        ...
        ({ template, nodeKey }) => {
          editor.update(() => {
            ...
            if (container == null || !$isLayoutContainerNode(container)) {
              return;
            }

            const items = getItemsFromTemplate(template);
            const itemsCount = items.length;
            const prevItemsCount = getItemsFromTemplate(container.getTemplateColumns()).length;
...
function getItemsFromTemplate(template: string): string[] {
